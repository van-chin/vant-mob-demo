...
import { StorageSerializers, useEventListener, useStorage, useStorageAsync, useTitle } from '@vueuse/core';
...
const { query } = route;
const { token, jwtkey, caccid, billName, billId, billTitle } = query;
...
function parseEditItems(editItems) {
  let editHeadItems = []
  let editBodyItems = []
...
(async function init() {
  ...
  console.info('init billTitle =>', billTitle);
  if (billTitle) {
  ...
  if (billId === undefined) {
    ...
    foreach(list.value.items[0].children, (item) => {
      if (item.component.vant) {
        ...
        if (item.component.vant.name === 'VzFormTable') {
          item.component.vant.props.columns.forEach((col, idx) => {
            console.info('list.value.modelKeys[item.id].model =>', list.value.modelKeys[item.id].model);
            if (bos[list.value.modelKeys[item.id].model] && !bos[list.value.modelKeys[item.id].model].includes(col.key)) {
              col.component.vant.props.disabled = true;
            }
          });
        }
